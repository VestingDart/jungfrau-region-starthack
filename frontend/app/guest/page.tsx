...
interface Guest { id: string; guest_card_id: string; email: string | null; check_in: string; check_out: string; }
interface Entitlement { id: string; title: string; description: string | null; partner_name: string; original_price_rappen: number | null; partner_payout_rappen: number; image_hint: string | null; }
interface PricedOffer { id: string; title: string; description: string | null; partner_name: string; partner_payout_rappen: number; original_price_rappen: number | null; image_hint: string | null; }
interface Transaction { type: string; amount_rappen: number; created_at: string; }
interface Wallet { balance_chf: number; guest: Guest; entitlements: Entitlement[]; available_priced_offers: PricedOffer[]; recent_transactions: Transaction[]; }
...
function fmt(d: string) { return new Date(d).toLocaleDateString('de-CH', { month: 'short', day: 'numeric' }); }
...
export default function GuestPage() {
  ...
  function signOut() { clearSession('guest'); router.push('/login'); }
  ...
  async function loadWallet(id?: string) {
    ...
    if (!gid) return;
    try { const w: Wallet = await apiFetch(`/api/wallet/${gid}`); setWallet(w); }
    catch (e: unknown) { showToast(t('common.error') + ': ' + (e as Error).message); }
  }

  async function topup() {
    if (!topupAmount || topupAmount <= 0) return showToast(t('common.error'));
    try {
  ...
  async function generateQR(eid: string | null, oid: string | null) {
    try {
      ...
      setQrTitle(t('guest.qrTitle'));
      setQrImg(`https://api.qrserver.com/v1/create-qr-code/?size=210x210&data=${encodeURIComponent(r.qr_token)}`);
      setCountdown(r.expires_in_seconds);
      setQrOpen(true);
      if (tickerRef.current) clearInterval(tickerRef.current);
      let s = r.expires_in_seconds;
      tickerRef.current = setInterval(() => {
        ...
        if (s <= 0) {
          if (tickerRef.current) clearInterval(tickerRef.current);
          setQrOpen(false);
          showToast(t('guest.qrExpiredToast'));
          loadWallet();
        }
      }, 1000);
    } catch (e: unknown) { showToast(t('common.error') + ': ' + (e as Error).message); }
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent) { if (e.data === 'topup_complete') loadWallet(); }
  ...
  const cbValidity = g ? `${t('guest.cardValidity')}: ${fmt(g.check_in)} – ${fmt(g.check_out)}, 2026` : `${t('guest.cardValidity')}: —`;
  ...
  return (
    <>
      <style>{`
        ...
        .offer-card { transition: transform .28s var(--ease), box-shadow .28s var(--ease); will-change: transform; }
        .offer-card:hover { transform: translateY(-6px); box-shadow: 0 20px 56px rgba(14,28,46,.18) !important; }
        .offer-card .card-img { transition: transform .5s var(--ease); }
        .offer-card:hover .card-img { transform: scale(1.06); }
        .use-btn { transition: background .2s, transform .15s var(--ease) !important; }
        .use-btn:not(:disabled):hover { background: var(--blue) !important; transform: translateY(-1px); }
        .stat-chip { transition: transform .22s var(--ease), box-shadow .22s var(--ease); cursor: default; }
      ...
      <nav className="navbar" style={{ position: 'fixed', top: 0, left: 0, right: 0 }}>
        <div className="nav-logo">Jungfrau<em style={{ color: 'var(--gold)' }}>.</em>Pass</div>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '2rem', zIndex: 2 }}>
          <div style={{ animation: 'fadeUp .65s var(--ease) both', animationDelay: '.08s' }}>
            <span style={{ display: 'inline-block', background: 'var(--gold)', color: '#fff', padding: '.22rem .9rem', borderRadius: 24, fontSize: '.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.16em', marginBottom: '1.35rem', boxShadow: '0 3px 18px rgba(196,149,14,.45)' }}>
              {t('guest.welcomeTag')}, {session?.name}
            </span>
          </div>
          <h1 style={{ fontSize: 'clamp(2.6rem,6.5vw,4.6rem)', fontWeight: 900, color: '#fff', lineHeight: 1.04, letterSpacing: '-.04em', textShadow: '0 4px 36px rgba(14,28,46,.7)', maxWidth: 660, margin: 0, animation: 'fadeUp .7s var(--ease) both', animationDelay: '.18s', whiteSpace: 'pre-line' }}>
            {t('guest.heroTitle')}
          </h1>
          ...
          {wallet && (
            <div style={{ marginTop: '2.25rem', display: 'inline-flex', alignItems: 'center', gap: '.9rem', background: 'rgba(255,255,255,.1)', backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)', padding: '.8rem 1.6rem', borderRadius: 50, border: '1px solid rgba(255,255,255,.18)', animation: 'fadeUp .7s var(--ease) both', animationDelay: '.42s' }}>
              <span style={{ fontSize: '.68rem', color: 'rgba(255,255,255,.55)', fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase' }}>{t('guest.myWallet')}</span>
              <span style={{ fontSize: '1.45rem', fontWeight: 900, color: '#fff', letterSpacing: '-.025em' }}>CHF {(wallet.balance_chf ?? 0).toFixed(2)}</span>
              <button onClick={() => setWalletOpen(true)} style={{ background: 'var(--gold)', color: '#fff', border: 'none', padding: '.32rem .9rem', borderRadius: 20, fontWeight: 700, fontSize: '.7rem', cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 2px 10px rgba(196,149,14,.4)' }}>
                {t('guest.topUp')} →
              </button>
            </div>
          )}
      ...
      <div style={{ background: '#F2EFE8', padding: '0 2rem 3.5rem' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: '1rem' }}>
          {[
            { label: t('guest.stat.elevation'),   value: '3,454 m',  sub: 'Jungfraujoch' },
            { label: t('guest.stat.skiTerrain'),   value: '213 km',   sub: t('guest.stat.skiSub') },
            { label: t('guest.stat.experiences'),  value: `${(wallet?.entitlements?.length || 0) + (wallet?.available_priced_offers?.length || 0)}`, sub: t('guest.stat.expSub') },
            { label: t('guest.stat.yourStay'),     value: wallet?.guest ? `${fmt(wallet.guest.check_in)} – ${fmt(wallet.guest.check_out)}` : '…', sub: '2026' },
          ].map((s, i) => (
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem 5rem' }}>
        ...
        <section style={{ marginBottom: '4.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1.25rem', marginBottom: '2rem' }}>
            <div style={{ flexShrink: 0 }}>
              <div style={{ fontSize: '.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.15em', color: 'var(--gold)', marginBottom: '.35rem' }}>{t('guest.includedSub')}</div>
              <h2 style={{ fontSize: '1.55rem', fontWeight: 900, color: 'var(--night)', letterSpacing: '-.03em', lineHeight: 1 }}>{t('guest.includedStay')}</h2>
              <p style={{ fontSize: '.875rem', color: 'var(--sub)', marginTop: '.35rem', lineHeight: 1.6 }}>{t('guest.includedDesc')}</p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(290px,1fr))', gap: '1.5rem' }}>
            {!wallet
              ...
              ? <p style={{ color: 'var(--sub)', fontSize: '.875rem', padding: '1rem 0' }}>{t('guest.allUsed')}</p>
                ...
                return (
                  <div key={e.id} className="offer-card" style={{ background: 'var(--white)', borderRadius: 'var(--r)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', animation: 'rise .52s var(--ease) both', animationDelay: `${i * .08}s`, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ position: 'relative', height: 205, overflow: 'hidden', background: 'var(--sand)' }}>
                      ...
                      <span style={{ position: 'absolute', top: '.75rem', left: '.75rem', background: 'var(--gold)', color: '#fff', padding: '.22rem .65rem', borderRadius: 20, fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.09em', boxShadow: '0 2px 10px rgba(196,149,14,.35)' }}>{t('guest.included')}</span>
                      <span style={{ position: 'absolute', top: '.75rem', right: '.75rem', background: 'var(--pine)', color: '#fff', padding: '.22rem .6rem', borderRadius: 20, fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em' }}>{t('guest.free')}</span>
                    </div>
                    <div style={{ padding: '1.2rem 1.25rem 1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '.5rem', marginBottom: '.9rem' }}>
                        <span style={{ fontSize: '.78rem', color: 'var(--sub)', textDecoration: 'line-through' }}>CHF {val} {t('guest.chfValue')}</span>
                        <span style={{ fontSize: '1.1rem', fontWeight: 900, color: 'var(--pine)', letterSpacing: '-.01em' }}>{t('guest.free').toUpperCase()}</span>
                      </div>
        ...
        <section style={{ marginBottom: '4.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '1.25rem', marginBottom: '2rem' }}>
            <div style={{ flexShrink: 0 }}>
              <div style={{ fontSize: '.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.15em', color: 'var(--blue)', marginBottom: '.35rem' }}>{t('guest.offersSub')}</div>
              <h2 style={{ fontSize: '1.55rem', fontWeight: 900, color: 'var(--night)', letterSpacing: '-.03em', lineHeight: 1 }}>{t('guest.availableOffers')}</h2>
              <p style={{ fontSize: '.875rem', color: 'var(--sub)', marginTop: '.35rem', lineHeight: 1.6 }}>{t('guest.offersDesc')}</p>
            </div>
            <div style={{ flex: 1, height: 1, background: 'var(--line)', marginBottom: '1.6rem' }} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(290px,1fr))', gap: '1.5rem' }}>
            {!wallet
              ...
              ? <p style={{ color: 'var(--sub)', fontSize: '.875rem', padding: '1rem 0' }}>{t('guest.noPricedOffers')}</p>
                ...
                return (
                  <div key={o.id} className="offer-card" style={{ background: 'var(--white)', borderRadius: 'var(--r)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', animation: 'rise .52s var(--ease) both', animationDelay: `${i * .08}s`, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ position: 'relative', height: 205, overflow: 'hidden', background: 'var(--sand)' }}>
                      ...
                      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top,rgba(14,28,46,.38) 0%,transparent 55%)' }} />
                      <span style={{ position: 'absolute', top: '.75rem', left: '.75rem', background: 'rgba(14,28,46,.72)', backdropFilter: 'blur(6px)', color: '#fff', padding: '.22rem .6rem', borderRadius: 20, fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em' }}>{t('guest.walletBadge')}</span>
                      {disc > 0 && <span style={{ position: 'absolute', top: '.75rem', right: '.75rem', background: 'var(--gold)', color: '#fff', padding: '.22rem .6rem', borderRadius: 20, fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em', boxShadow: '0 2px 10px rgba(196,149,14,.35)' }}>{disc}% {t('guest.off').toUpperCase()}</span>}
                    </div>
                    <div style={{ padding: '1.2rem 1.25rem 1.5rem', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      ...
                      <button className="use-btn" onClick={() => ok ? generateQR(null, o.id) : undefined} disabled={!ok} style={{ width: '100%', border: 'none', borderRadius: 9, padding: '.7rem 1rem', fontWeight: 700, fontSize: '.875rem', cursor: ok ? 'pointer' : 'not-allowed', background: ok ? 'var(--navy)' : 'var(--line)', color: ok ? '#fff' : 'var(--sub)', fontFamily: 'inherit' }}>
                        {ok ? `${t('guest.buyWith')} →` : t('guest.insufficientBalance')}
                      </button>
                    </div>
                  </div>
                );
        ...
        <section>
          <div style={{ background: 'var(--white)', borderRadius: 'var(--r)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
            <div style={{ padding: '1.1rem 1.5rem', borderBottom: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--night)' }}>
              <div>
                <div style={{ fontSize: '.58rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.14em', color: 'rgba(255,255,255,.38)', marginBottom: '.15rem' }}>{t('guest.myWallet')}</div>
                <h2 style={{ fontSize: '.82rem', fontWeight: 700, color: '#fff' }}>{t('guest.txHistory')}</h2>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={() => loadWallet()} style={{ color: 'rgba(255,255,255,.55)', border: '1px solid rgba(255,255,255,.14)' }}>{t('common.refresh')}</button>
            </div>
            <div style={{ padding: '.35rem 1.5rem' }}>
              {!wallet?.recent_transactions?.length
                ? <div style={{ textAlign: 'center', padding: '2.5rem 1rem', color: 'var(--sub)', fontSize: '.875rem' }}>{t('guest.noTx')}</div>
                : wallet.recent_transactions.slice(0, 10).map((tx, i) => {
                  const a = tx.amount_rappen / 100;
                  const s = a >= 0 ? '+' : '';
                  const d = new Date(tx.created_at).toLocaleString('de-CH', { dateStyle: 'short', timeStyle: 'short' });
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '.78rem 0', borderBottom: '1px solid var(--line)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 600, fontSize: '.85rem', textTransform: 'capitalize', color: 'var(--text)' }}>{tx.type}</div>
      ...
      <div style={{ position: 'fixed', top: 0, right: walletOpen ? 0 : -420, width: 390, height: '100vh', background: 'var(--white)', boxShadow: '-8px 0 64px rgba(14,28,46,.28)', zIndex: 61, transition: 'right .38s cubic-bezier(.4,0,.2,1)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(160deg,var(--night) 0%,#1B3259 100%)', padding: '1.5rem', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.35rem' }}>
            <div>
              <div style={{ fontSize: '.58rem', textTransform: 'uppercase', letterSpacing: '.14em', color: 'rgba(255,255,255,.38)', fontWeight: 700, marginBottom: '.2rem' }}>{t('guest.myWallet')}</div>
          ...
          <div style={{ fontSize: '.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.12em', color: 'rgba(255,255,255,.38)', marginBottom: '.3rem' }}>{t('guest.availableBalance')}</div>
          <div style={{ fontSize: '2.7rem', fontWeight: 900, letterSpacing: '-.045em', color: '#fff', lineHeight: 1 }}>CHF {(wallet?.balance_chf ?? 0).toFixed(2)}</div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--line)' }}>
            <div style={{ fontSize: '.62rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.1em', color: 'var(--sub)', marginBottom: '.65rem' }}>{t('guest.topUpVia')}</div>
            ...
            <div style={{ display: 'flex', gap: '.5rem' }}>
              ...
              <button onClick={topup} style={{ background: 'var(--gold)', color: '#fff', border: 'none', padding: '.62rem 1.1rem', borderRadius: 8, fontWeight: 700, fontSize: '.82rem', cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit', boxShadow: '0 2px 12px rgba(196,149,14,.35)' }}>{t('guest.topUp')}</button>
            </div>
          </div>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--line)' }}>
              ...
              <div style={{ position: 'relative', width: '100%', height: '100%', transition: 'transform .68s cubic-bezier(.4,0,.2,1)', transformStyle: 'preserve-3d', transform: cardFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
                <div style={{ position: 'absolute', inset: 0, borderRadius: 16, overflow: 'hidden', backfaceVisibility: 'hidden', boxShadow: '0 10px 40px rgba(14,28,46,.32)', background: 'var(--navy)' }}>
                  ...
                  <div style={{ position: 'relative', zIndex: 1, height: '100%', padding: '1.1rem 1.3rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', background: 'linear-gradient(140deg,rgba(14,28,46,.75) 0%,rgba(14,28,46,.15) 100%)', color: '#fff' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '1.12rem', fontWeight: 900, letterSpacing: '.02em', marginBottom: '.2rem' }}>{t('guest.guestPass')}</div>
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            {!wallet?.recent_transactions?.length
              ? <div style={{ textAlign: 'center', padding: '1.5rem 0', color: 'var(--sub)', fontSize: '.875rem' }}>{t('guest.noActivity')}</div>
              : wallet.recent_transactions.slice(0, 8).map((tx, i) => {
                const a = tx.amount_rappen / 100;
                const s = a >= 0 ? '+' : '';
                const d = new Date(tx.created_at).toLocaleString('de-CH', { dateStyle: 'short', timeStyle: 'short' });
                return (
        ...
        <div onClick={closeModal} style={{ position: 'fixed', inset: 0, background: 'rgba(14,28,46,.78)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--white)', borderRadius: 24, padding: '2rem', textAlign: 'center', maxWidth: 400, width: '100%', boxShadow: 'var(--shadow-lg)', animation: 'rise .25s var(--ease)' }}>
            ...
            <div style={{ fontSize: '.85rem', color: 'var(--sub)' }}>
              {t('guest.qrExpires')} <span style={{ color: countdown < 15 ? 'var(--danger)' : 'var(--pine)', fontWeight: 800 }}>{countdown}</span>s
            </div>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <p style={{ fontSize: '.72rem', color: 'var(--sub)' }}>{t('guest.qrCopyHint')}</p>
            ...
            <div style={{ display: 'flex', gap: '.5rem', marginTop: '1rem', justifyContent: 'center' }}>
              <button onClick={copyToken} className="btn btn-ghost" style={{ padding: '.58rem 1.1rem', fontSize: '.875rem' }}>{t('guest.qrCopyBtn')}</button>
              <button onClick={closeModal} className="btn btn-primary" style={{ padding: '.58rem 1.1rem', fontSize: '.875rem' }}>{t('common.close')}</button>
            </div>
          </div>
        </div>
      )}
