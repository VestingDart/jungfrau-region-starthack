      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-2xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-4">
          ...
          <p className="text-sm text-gray-500 mt-1.5 leading-relaxed">
            ...
            Valid for <span className="font-medium text-gray-700">15 minutes</span>, one-time use only.
          </p>
        </div>
...
export default function PassView({ data, username }: { data: PassResponse; username: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#F2F6F4]">
      ...
      <header className="bg-brand border-b border-brand-dark px-6 py-3.5 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={logout}
            className="text-xs font-medium border border-white/30 rounded-lg px-3 py-1.5 text-white/80 hover:bg-white/10 transition-colors">
            Sign out
          </button>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-5 py-5 transition-all duration-200 hover:shadow-md hover:border-brand-border hover:-translate-y-0.5">
            ...
            <p className="text-2xl font-bold text-gray-900 tracking-tight">CHF {pass.walletBalanceChf.toFixed(2)}</p>
            <p className="text-xs text-gray-400 mt-1 mb-4">Hotel deposit · cashback eligible</p>
            <div className="flex gap-2">
              <button className="flex-1 border border-gray-200 rounded-lg py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors">Top up</button>
              <button className="flex-1 border border-gray-200 rounded-lg py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 transition-colors">Transactions</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-5 py-5 transition-all duration-200 hover:shadow-md hover:border-brand-border hover:-translate-y-0.5">
            ...
            <p className="text-xs text-gray-400 mt-1 mb-4">
              {redeemedCount > 0 ? `${redeemedCount} redeemed · ` : ''}{alwaysActive.length} always included
            </p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 px-5 py-5 transition-all duration-200 hover:shadow-md hover:border-brand-border hover:-translate-y-0.5">
            ...
            <p className="text-xs text-gray-400 mt-1 mb-4">{100 - pass.sustainabilityPoints} more until next reward</p>
            ...
            <p className="text-xs text-gray-400 mt-2">Earned via BOB train + hiking</p>
          </div>
                ...
                {sortedOneTime.map((benefit) => {
                  ...
                  return (
                    <tr key={benefit.id} className={`transition-colors ${isRedeemed ? '' : 'hover:bg-gray-50'}`}>
                      <td className={`px-6 py-3.5 font-medium ${isRedeemed ? 'text-gray-400' : 'text-gray-900'}`}>{benefit.title}</td>
                      <td className={`px-4 py-3.5 ${isRedeemed ? 'text-gray-400' : 'text-gray-500'}`}>{benefit.partnerName}</td>
                      <td className={`px-4 py-3.5 ${isRedeemed ? 'text-gray-400' : 'text-gray-500'}`}>{benefit.description}</td>
      ...
      {modal?.phase === 'confirm' && activeBenefit && (
        <ConfirmModal benefit={activeBenefit} onConfirm={() => confirmRedemption(activeBenefit)} onClose={() => setModal(null)} />
      )}
      {modal?.phase === 'active' && activeBenefit && activeRedemption && (
        <RedemptionModal benefit={activeBenefit} redemption={activeRedemption} onRotate={() => rotateToken(activeBenefit)} onClose={() => setModal(null)} />
      )}
    </div>
  );
}
